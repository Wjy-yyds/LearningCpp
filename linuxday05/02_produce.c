...
void * producethread1(void *arg){
    ...
    while(1){
        ...
        while(pshareRes->size>=MAXSIZE){
            ...
            pthread_cond_wait(&pshareRes->produce_cond,&pshareRes->mutex);
    ...
    //通知消费线程
    pthread_cond_broadcast(&pshareRes->consume_cond);
...
void * consumerthread1(void *arg){
    ...
    while(pshareRes->size<=MINSIZE){
        ...
        pthread_cond_wait(&pshareRes->consume_cond,&pshareRes->mutex);
        printf("consumer1 is wake up now!\n");
    }
    //--pshareRes->size;
    int dtn=headDelete(pshareRes);
    pthread_cond_broadcast(&pshareRes->produce_cond);
...
int main(){
    ...
    pthread_cond_init(&shareRes.produce_cond,NULL);
    pthread_cond_init(&shareRes.consume_cond,NULL);
