...
int main(int argc, char *argv[]){                                  
    ...
    char usernames[1024][32]; 
    for(int i = 0; i < 1024; ++i){
        nret[i] = -1;
        snprintf(usernames[i], sizeof(usernames[i]), "user%d", i);
    }
    while(1) {
                ...
                if(buf[0]=='@'){
                    ...
                    *message='\0';
                    ...
                    for(int j=0;j<curindex;j++){
                        if(nret[j]!=-1&& !strcmp(usernames[j],target)){
                            char privatemsg[1024];
                            snprintf(privatemsg,sizeof(privatemsg),"私%s:%s",usernames[i],msg);
                            send(nret[j],privatemsg,strlen(privatemsg),0);
                            break;
                        }
                    }
